...
species <- c("Erythroxylon monogynum","Flacourtia sepiaria","Lantana camara","Randia dumetorum","Wrightia tinctoria","Albizzia amara","Azadirachta indica","Strychnos nux-vomica","Tamarindus indicus","Acacia leucophloea","Pongamia pinnata","Delonix regia")

png(units="in", width=12, height=8, res=300, file = paste('C:/Users/Nandu/Desktop/Thesis/NCF/R/Thesis plots/flower rank.png'))    

a <- layout(matrix(c(1:12),3,4))

par(mar = c(1,2,2,1)+0.1, mgp = c(1.5,0.5,0), oma = c(5,2,1,1)+0.1, xpd = F)
...
for (s in 1:length(species)) {
  ...
  for (k in 2:7) {  # k index for specifying year
    ...
    for (j in 1:length(levels(factor(use.sp_dat$tag_id)))) {                                                 # j index for individuals
      ...
      if (is.na(use.sp_dat_year_ind$flower[1]) == TRUE){           #Assigning NA to years in which certain species have not been recorded
        
        flowering_rank[,k][j] <- NA
        
      } else { 
        for (i in 1:length(use.sp_dat_year_ind$datep)){                 # i index for the number of observations in a year      
          if (use.sp_dat_year_ind$flower[i] == 'Y'){
  ...
  flowering_rank <- flowering_rank[rowSums(is.na(flowering_rank)) < 2, ]  #eliminate rows with more than one NA
  ...
  real_median <- apply(flowering_rank[,2:7],1,median, na.rm = TRUE)  #calculating the medians of the actual data
...
legend(-120,-13,c('Mean Simulated Medians', '2.5 & 97.5 quantile simulated median', 'Observed Medians'),lty = c(1,2,1), lwd = c(2,2,2), col = c('black','black','red'), horiz =T) 
