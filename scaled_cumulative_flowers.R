...
species <- c("Erythroxylon monogynum","Flacourtia sepiaria","Lantana camara","Randia dumetorum","Wrightia tinctoria","Albizzia amara","Azadirachta indica","Strychnos nux-vomica","Tamarindus indicus","Acacia leucophloea","Pongamia pinnata","Delonix regia", 'Santalum album', 'Chomelia asiatica')

png(units="in", width=8.3, height=11.7, res=300, file = paste('C:/Users/Abinand Reddy/Desktop/Thesis/NCF/R/Thesis plots/scaled cumulative flower.png'))    

fsp_st <- data.frame(matrix(NA, ncol = 2, nrow = length(species)))
rownames(fsp_st) <- species
colnames(fsp_st) <- c('synchrony', 'consistency')

a <- layout(matrix(c(1:15),5,3))

par(mar = c(1,2,2,1)+0.1, mgp = c(1.5,0.5,0), oma = c(4,2,1,1)+0.1)
...
for (s in 1:length(species)) {
  ...
  for ( i in 3:8 ){
    ...
    if(is.na(flowering_day[1,i]) == F) quant_leaf[i-2] <- sync(flowering_day[,i],julian(as.Date(names(datelist[4:27])),as.Date(names(datelist))[4]))
  }  
  fsp_st$synchrony[s] <- mean(quant_leaf, na.rm = T)
  fsp_st$consistency[s] <- max(mid_leaf, na.rm = T) - min(mid_leaf, na.rm = T)
}
par(xpd = NA)
...
title(xlab = "Days",ylab = "Scaled cumulative flowering",outer = TRUE, line = 1, cex = 1.5)
dev.off()

par(mfrow = c(1,1), mar = c(7,4,2,2))
fsp_st
fsynco_sp_st <- fsp_st[order(-fsp_st$synchrony),]

x <- barplot(fsynco_sp_st$synchrony)

text(cex=1, x=x+.25, y=-1.25, rownames(fsynco_sp_st), xpd=TRUE, srt= 45, pos=2)

fsynco_sp_st <- fsp_st[order(-fsp_st$consistency),]

x <- barplot(fsynco_sp_st$consistency)
...
synco_sp_st$species <- rownames(synco_sp_st)
fsynco_sp_st$species <- rownames(fsynco_sp_st)

synco_sp_st
fsynco_sp_st

combined <- merge(synco_sp_st,fsynco_sp_st, by = 'species', all = T)

synco_sp_st
fsynco_sp_st

synsp <- c("S. nux-vomica" ,"F. benghalensis","W. tinctoria","P. pinnata","F. religiosa","R. dumetorum","F. sepiaria","E. monogynum" ,"A. amara","A. leucophloea","A. indica","T. indicus","L. camara","D. regia","C. asiatica","S. album")

png(units="in", width=10, height=6, res=300, file = paste('C:/Users/Abinand Reddy/Desktop/Thesis/NCF/R/Thesis plots/synchrony.png'))    

par(mfrow= c(1,2), mar= c(5,4,0,1), xpd = NA, oma = c(1,0,4,1))

combined <- combined[order(-combined$synchrony.x),]
combined$species <- synsp
x <- barplot(combined$synchrony.x[1:14], col = 'dark green', ylim = c(0,70), ylab = 'Leaf Flush Synchrony (Days)')
text(cex=1, x = x-.5 ,  y = -4, combined$species[1:14], xpd=NA, srt= -45, pos = 4.5)

combined <- combined[order(-combined$synchrony.y),]
x <- barplot(combined$synchrony.y[1:14], col = 'dark red', ylim = c(0,70), ylab = 'Flowering Synchrony (Days)', xpd = NA)
text(cex=1, x = x-.5 ,  y = -4, combined$species[1:14], xpd=NA, srt= -45, pos = 4.5)
title('Synchrony', outer = T)

dev.off()

png(units="in", width=10, height=6, res=300, file = paste('C:/Users/Abinand Reddy/Desktop/Thesis/NCF/R/Thesis plots/consistency.png'))    

par(mfrow= c(1,2), mar= c(5,4,0,1), xpd = NA, oma = c(1,0,4,1))

combined <- combined[order(-combined$consistency.x),]
x <- barplot(combined$consistency.x[1:14], col = 'dark green', ylim = c(0,120), ylab = 'Leaf Flush Consistency (Days)')
text(cex=1, x = x-.5 ,  y = -6, combined$species[1:14], xpd=NA, srt= -45, pos = 4.5)

combined <- combined[order(-combined$consistency.y),]
x <- barplot(combined$consistency.y[1:14], col = 'dark red', ylim = c(0,120), ylab = 'Flowering Consistency (Days)')
text(cex=1, x = x-.5 ,  y = -6, combined$species[1:14], xpd=NA, srt= -45, pos = 4.5)
title('Consistency', outer = T)
